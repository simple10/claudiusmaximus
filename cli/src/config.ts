...
const REQUIRED_FIELDS = ['VPS1_IP', 'SSH_KEY_PATH', 'SSH_USER', 'SSH_PORT'] as const
...
function findProjectRoot(): string {
  ...
  throw new Error('Cannot find openclaw-config.env. Run the CLI from the openclaw-vps project.')
}
...
export function loadConfig(): Config {
  ...
  if (missing.length > 0) {
    throw new Error(`Missing required fields in openclaw-config.env: ${missing.join(', ')}`)
  }
