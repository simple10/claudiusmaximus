...
The OpenClaw repo includes a docker-compose.yml. Create an override file to add monitoring and security settings:
...
EOF
    ...
    # No domain — serve on IP (HTTP works, HTTPS with self-signed has browser warnings)
    sudo tee /etc/caddy/Caddyfile << 'EOF'
{
    local_certs
}
...
1. Access OpenClaw dashboard via `http://<VPS1-IP>` (or `https://<DOMAIN>` if configured)
2. Access Grafana via `http://<VPS2-IP>` (login: admin / generated password)
3. Verify Prometheus targets in Grafana → Explore → Prometheus
4. Check logs flowing from VPS-1 in Grafana → Explore → Loki
...
9. **Caddy TLS**: Use `local_certs` global option for self-signed certificates
